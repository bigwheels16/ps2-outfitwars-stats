...
class Service:
    ...
    def get_damage(self, version, weapons, targets, directions):
        ...
        if weapons:
            sql += " AND ("
            sql += " OR ".join([f"w.weapon_id = :weapon_id_{q}" for q in weapons])
            sql += ")"
            for q in weapons:
                params[f"weapon_id_{q}"] = q

        if targets:
            sql += " AND ("
            sql += " OR ".join([f"t.target_id = :target_id_{q}" for q in targets])
            sql += ")"
            for q in targets:
                params[f"target_id_{q}"] = q

        if directions:
            sql += " AND ("
            sql += " OR ".join([f"a2.id = :direction_{q}" for q in directions])
            sql += ")"
            for q in directions:
                params[f"direction_{q}"] = q
